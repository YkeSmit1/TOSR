    ...
    public class BidManager
    {
        ...
        private Bid CalculateEndContract(Auction auction, string northHand, Bid currentBid)
        {
            ...
            bool TryGetEndContract(Suit triedSuit, out Bid bid, out int confidenceInContract)
            {
                ...
                loggerBidding.Info($"Tried bid:{bid}. Confidence in CalculateEndContract:{JsonConvert.SerializeObject(confidenceInContract)} with matches:{string.Join(",", constructedSouthHands)} " +
                    $"HCP:{hcp?.Min}-{hcp?.Max} queens:{queens} trumpSuit:{triedSuit}");
